...
// - `supabaseAdmin` uses the service role key when available (required for admin actions)
// - `supabase` is an anon client used for public auth actions like signUp/signIn
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // Step 1: Create user in Supabase Auth using anon client (no service role required)
    // Frontend can also call `supabase.auth.signUp` directly; here we call signUp server-side
...
app.post('/api/check-email-exists', async (req, res) => {
  try {
    ...
    // If DNS / network issue prevents DB lookup, return a safe fallback so frontend can continue.
...
app.post('/api/driver-application', async (req, res) => {
  try {
    ...
    const existing = await pool.query('SELECT id FROM driver_applications WHERE user_id = $1', [
      userId,
    ]);
...
app.post('/api/admin/approve-student', requireAdmin, async (req, res) => {
  try {
    ...
    await pool.query('UPDATE users SET is_verified = TRUE WHERE id = $1', [studentId]);
...
app.get('/api/admin/tickets', requireAdmin, async (req, res) => {
  try {
    ...
    const r = await pool.query(
      `SELECT t.*, COALESCE(u.full_name, u.username) as username, u.email FROM tickets t LEFT JOIN users u ON t.user_id = u.id ${where} ORDER BY t.created_at DESC`,
      params
    );
    res.json({ tickets: r.rows });
  } catch (_) {
...
app.post('/api/admin/tickets/:ticketId/review', requireAdmin, async (req, res) => {
  try {
    ...
    const upd = await pool.query(
      ...
      [status, adminReply || null, ticketId]
    );
...
app.post('/api/admin/review-driver', requireAdmin, async (req, res) => {
  try {
    ...
    const appRes = await pool.query('SELECT * FROM driver_applications WHERE id = $1', [
      applicationId,
    ]);
      ...
      try {
        ...
        if (socketId && io && io.to) {
          try {
            ...
            console.log(
              `Emitted driverStatusUpdated to user ${userId} at socket ${socketId}`,
              payload
            );
            delivered = true;
          } catch (_) {
...
app.post('/api/rides/:rideId/cancel', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE rides SET status = $1, cancelled_at = NOW() WHERE id = $2', [
      'cancelled',
      rideId,
    ]);
...
app.post('/api/rides/:rideId/end', async (req, res) => {
  try {
    ...
    // Mark completed
    await pool.query('UPDATE rides SET status = $1, completed_at = NOW() WHERE id = $2', [
      'completed',
      rideId,
    ]);
    ...
    try {
      ...
      const parsedFare =
        fare !== undefined ? parseFloat(fare) : ride.fare ? parseFloat(ride.fare) : 0;
      if (driverId && parsedFare) {
        await pool.query(
          'UPDATE driver_applications SET total_rides = COALESCE(total_rides,0) + 1, total_earnings = COALESCE(total_earnings,0) + $1 WHERE user_id = $2',
          [parsedFare, driverId]
        );
      }
    } catch (_) {
  ...
  try {
    ...
    try {
      const avgRes = await pool.query(
        'SELECT AVG(rating) as avg_rating, COUNT(*) as cnt FROM reviews WHERE target_id = $1',
        [targetId]
      );
...
io.on('connection', (socket) => {
  ...
  socket.on('driverOnline', async (data) => {
    ...
    await pool.query('UPDATE driver_applications SET is_online = TRUE WHERE user_id = $1', [
      driverId,
    ]);
  ...
  socket.on('driverOffline', async (driverId) => {
    ...
    await pool.query('UPDATE driver_applications SET is_online = FALSE WHERE user_id = $1', [
      driverId,
    ]);
  ...
  socket.on('chatMessage', async (data) => {
    ...
    await pool.query('INSERT INTO messages (ride_id, sender_id, message) VALUES ($1, $2, $3)', [
      rideId,
      senderId,
      message,
    ]);
...
server.on('error', (err) => {
  if (err && err.code === 'EADDRINUSE') {
    console.error(
      `✋ Port ${PORT} is already in use. Stop the other process or set a different PORT environment variable.`
    );
    console.error(
      'Tip: run `lsof -iTCP:3000 -sTCP:LISTEN -Pn` to find the PID, then `kill <PID>` to free the port.'
    );
    process.exit(1);
  }
